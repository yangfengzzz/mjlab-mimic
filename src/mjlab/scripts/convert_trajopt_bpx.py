"""Convert BPX TrajOpt JSON solutions to mjlab motion ``.npz`` files.

Input JSON format:
  ``solution.nodes[*].q`` is ``pos3 + quat_xyzw4 + 12 BPX joints``.
...
from mjlab.tasks.tracking.config.bpx.env_cfgs import bpx_flat_tracking_env_cfg
...
class TrajoptBpxMotionLoader:
  """Load and resample a BPX TrajOpt solution."""
  ...
  def _interpolate_motion(self) -> None:
    ...
    print(
      f"Loaded BPX TrajOpt motion: {self.motion_base_poss_input.shape[0]} nodes "
...
def _upload_to_wandb(output_file: Path, wandb_name: str) -> None:
  ...
  run = wandb.init(project="trajopt_bpx", name=wandb_name)
  registry = "motions"
...
def convert_file(
  input_file: str,
  output_file: str = "/tmp/bpx_motion.npz",
  ...
  """Convert a BPX TrajOpt JSON solution to a train-ready ``motion.npz``."""
  if device.startswith("cuda") and not torch.cuda.is_available():
    print("[WARNING]: CUDA is not available. Falling back to CPU.")
    device = "cpu"

  scene = Scene(bpx_flat_tracking_env_cfg().scene, device=device)
  model = scene.compile()
...
def run_sim(
  ...
  robot_joint_indexes = robot.find_joints(BPX_JOINT_NAMES, preserve_order=True)[0]
  ...
  pbar = tqdm(
    ...
    desc="Processing BPX TrajOpt frames",
    unit="frame",
    ncols=100,
  ...
  print(f"[INFO]: Saved BPX motion to {output_file}")
  ...
  if render and frames:
    ...
    print(f"[INFO]: Saved BPX motion video to {video_path}")
...
def main(
  ...
  """Convert a BPX TrajOpt JSON solution to a local ``motion.npz`` file."""
  convert_file(
    input_file=input_file,
